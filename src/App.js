...
function App() {
  ...
  return (
    <>
      ...
      <div>
        {(walletsRedux !== null && walletsRedux.wallets[1] !== undefined) &&
          walletsRedux.wallets.map((wlt, index) => {
            if (index !== 0) {
              var arrayUrls = []
              wlt.wallet_nfts.map((nft, indexnft) => {
                const image = walletsRedux.wallets[index].wallet_nfts[indexnft].data.img;
